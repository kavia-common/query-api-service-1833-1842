...
- Requirements (conditional)
  - Install `psycopg2-binary` only if you use PostgreSQL:
    - Option A (preferred for Postgres users): uncomment `psycopg2-binary` in `sql_query_backend/requirements.txt` and then `pip install -r sql_query_backend/requirements.txt`
    - Option B: `pip install psycopg2-binary`
- Run (choose one)
...
1) Install dependencies (prefer virtualenv/venv)
   ...
   - PostgreSQL users only: ensure `psycopg2-binary` is installed (uncomment it in requirements or `pip install psycopg2-binary`)
...
- OpenAPI is auto-generated by FastAPI at runtime at `/openapi.json`. A snapshot is stored in `interfaces/openapi.json` and `sql_query_backend/interfaces/openapi.json`.